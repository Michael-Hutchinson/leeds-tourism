...
class View extends CI_Controller {
        protected $see_form_validation = array(
    ...
    public function index()
    {
        //Load user model
        $this->load->model('see_model');
        $data['rows'] = $this->see_model->get_all_see();
   ...
   else
   {
     ...
     redirect('login', 'refresh');
        ...
        $this->load->view('admin_dashboard/pages/see/view_all_see');
    ...
    public function edit($id = null)
    {
            //Load user model
            $this->load->model('see_model');
            $data['row'] = $this->see_model->get_see($id);
            $data['left_nav_headings'] = $this->see_model->get_left_nav_headings();

        //Set form validation
        $this->form_validation->set_rules($this->see_form_validation);
        if($this->session->userdata('current_user'))
   {
     $session_data = $this->session->userdata('current_user');
     $data['session_data'] = $session_data;
   ...
   else
   {
     ...
     redirect('login', 'refresh');
        ...
        if($this->form_validation->run() == false) 
            $this->load->view('admin_dashboard/pages/see/edit_see', $data);
        else {
            ...
            $this->see_model->edit_see($id,$section_category_id,$name,$title,$body);
            $this->session->set_flashdata('message', '1 entry updated!');

        redirect('admin/see/view');
}
        //Load Footer Tags
        $this->load->view('main/page_components/footer-tags');
   ...
   else
   {
     ...
     redirect('login', 'refresh');
            ...
            $this->load->model('see_model');
            $delete = $this->see_model->delete_see($id);
            redirect('admin/see/view/');
   ...
   else
   {
     ...
     redirect('login', 'refresh');
        ...
        if($this->form_validation->run() == false) 
            $this->load->view('admin_dashboard/pages/see/add_see');
        else {
            ...
            $this->see_model->add_see($name, $title, $body, $section_category_id);
            $this->session->set_flashdata('message', '1 new entry added!');

        redirect('admin/see/view/add');
}
        //Load Footer Tags
        $this->load->view('main/page_components/footer-tags');
